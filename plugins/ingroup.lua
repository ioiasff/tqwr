...
local function check_member_autorealm(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'به گروه مادر جدیدتان خوش امدید')
    end
  end
end
local function check_member_realm_add(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      data[tostring(realms)][tostring(msg.to.id)] = msg.to.id
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'گروه مادر به لیست گروه های مادر من اضافه شد')
    end
  end
end
...
local lock_sticker = "no"
  ...
  local text = "تنظیمات گروه ⚙ :\nقفل بودن اسم گروه🔘 : "..settings.lock_name.."\nقفل بودن عکس گروه 🖼 : "..settings.lock_photo.."\nقفل بودن تگ گروه🔺 : "..lock_tag.."\nقفل بودن دروازه گروه🔒🔓 : "..settings.lock_member.."\nاجازه انگلیسی حرف زدن 🗣 : "..lock_eng.."\n بن شدن کاربران لفتی : "..lock_leave.."\nقفل بودن کلمات رکیک💢 : "..lock_badw.."\nقفل بودن جلوگیری از لینک🔗 : "..lock_link.."\nقفل بودن ورود به گروه با لینک : "..lock_adds.."\nامکان ارسال استیکر ✴ : "..lock_sticker.."\nتعداد حساسیت پیام : "..NUM_MSG_MAX.."\nاجازه ورود ربات : "..bots_protection--"\nPublic: "..public
  return text
end
...
local function get_description(msg, data)
  ...
  if not data[tostring(msg.to.id)][data_cat] then
    return 'هیچ شرحی برای گروه گزاشته نشده.'
  end
  local about = data[tostring(msg.to.id)][data_cat]
  local about = string.gsub(msg.to.print_name, "_", " ")..':\n\n'..about
  return 'شرح گروه '..about
end
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return '.عربی نویسی و فارسی نویسی قبلا ممنوع شد.'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'عربی نویسی و فارسی نویسی ممنوع شد.'
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'عربی نویسی و فارسی نویسی قبلا ازاد شد.'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return 'عربی نویسی و فارسی نویسی ازاد شد.'
  end
end
...
local function lock_group_link(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    return 'ارسال لینک در گروه قبلا ممنوع شده'
  else
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'kick' then
    return 'ارسال استیکر در گروه قبلا ممنوع🚫شده'
  else
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'ok' then
    return 'ارسال استیکر در گروه قبلا ازاد شده'
  else
    data[tostring(target)]['settings']['sticker'] = 'ok'
    save_data(_config.moderation.data, data)
    return 'ارسال استیکر در گروه ازاد شد.'
  end
end
...
local function unlock_group_link(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return 'ارسال لینک قبلا در گروه ازاد شده. '
  else
...
local function lock_group_eng(msg, data, target)
  ...
  if group_eng_lock == 'yes' then
    return 'انگلیسی نویسی قبلا ممنوع🚫شده.'
  else
    data[tostring(target)]['settings']['lock_eng'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'انگلیسی نویسی ممنوع شد'
  end
end
...
local function unlock_group_eng(msg, data, target)
  ...
  if group_eng_lock == 'no' then
    return 'انگلیسی نویسی قبلا ازاد شده.'
  else
    data[tostring(target)]['settings']['lock_eng'] = 'no'
    save_data(_config.moderation.data, data)
    return 'انگلیسی نویسی ازاد شد'
  end
end
local function lock_group_eng(msg, data, target)
  if not is_momod(msg) then
  ...
  local group_eng_lock = data[tostring(target)]['settings']['lock_eng']
  if group_eng_lock == 'yes' then
    return 'انگلیسی نویسی قبلا در گروه ممنوع شده.'
  else
    data[tostring(target)]['settings']['lock_eng'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'انگلیسی نویسی در گروه ممنوع شد. '
  end
end
...
local function unlock_group_eng(msg, data, target)
  ...
  local group_eng_lock = data[tostring(target)]['settings']['lock_eng']
  if group_eng_lock == 'no' then
    return 'انگلیسی نویسی قبلا در گروه ازاد شده.'
  else
    data[tostring(target)]['settings']['lock_eng'] = 'no'
    save_data(_config.moderation.data, data)
    return 'انگلیسی نویسی در گروه ازاد شد.'
  end
end
...
local function lock_group_tag(msg, data, target)
  ...
  if group_tag_lock == 'yes' then
    return 'ارسال # در گروه قبلا ممنوع شده.'
  else
...
local function unlock_group_tag(msg, data, target)
  ...
  if group_tag_lock == 'no' then
    return 'ارسال # در گروه قبلا ازاد شده.'
  else
    data[tostring(target)]['settings']['lock_tag'] = 'no'
    save_data(_config.moderation.data, data)
    return 'ارسال # در گروه ازاد شد'
  end
end
local function lock_group_tag(msg, data, target)
  if not is_momod(msg) then
  ...
  local group_tag_lock = data[tostring(target)]['settings']['lock_tag']
  if group_tag_lock == 'yes' then
    return 'ارسال # قبلا در گروه ممنوع شده.'
  else
    data[tostring(target)]['settings']['lock_tag'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'ارسال # در گروه ممنوع شد.'
...
local function unlock_group_tag(msg, data, target)
  ...
  local group_tag_lock = data[tostring(target)]['settings']['lock_tag']
  if group_tag_lock == 'no' then
    return 'ارسال # قبلا در گروه ازاد شده.'
  else
    data[tostring(target)]['settings']['lock_tag'] = 'no'
    save_data(_config.moderation.data, data)
    return 'ارسال # در گروه ازاد شد.'
...
local function lock_group_badw(msg, data, target)
  ...
  if group_badw_lock == 'yes' then
    return 'جلوگیری از ارسال کلمات رکیک قبلا فعال شده.'
  else
    data[tostring(target)]['settings']['lock_badw'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'جلوگیری از ارسال کلمات رکیک فعال شد.'
  end
end
...
local function unlock_group_badw(msg, data, target)
  ...
  if group_badw_lock == 'no' then
    return 'جلوگیری از ارسال کلمات رکیک قبلا غیر فعال شده.'
  else
    data[tostring(target)]['settings']['lock_badw'] = 'no'
    save_data(_config.moderation.data, data)
    return 'جلوگیری از ارسال کلمات رکیک غیر فعال شد.'
  end
end
local function lock_group_badw(msg, data, target)
  if not is_momod(msg) then
...
local function lock_group_adds(msg, data, target)
  ...
  if adds_ban == 'yes' then
    return 'دوازه پیوستن اعضا جدید با لینک بسته شده.'
  else
    data[tostring(msg.to.id)]['settings']['adds_ban'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'دروازه پیوستن اعضا جدید با لینک قبلا بسته شده.'
end
...
local function unlock_group_adds(msg, data, target)
  ...
  local adds_ban = data[tostring(msg.to.id)]['settings']['adds_ban']
  if adds_ban == 'no' then
    return 'دروازه پیوستن اعضا جدید با لینک ازاد شد.'
  else
    data[tostring(msg.to.id)]['settings']['adds_ban'] = 'no'
    save_data(_config.moderation.data, data)
    return 'دروازه پیوستن اعضا جدید با لینک قبلا ازاد شده.'
  end
end
...
local function lock_group_leave(msg, data, target)
  ...
  if leave_ban == 'yes' then
    return 'از این به بعد کسانی که لفت میدهند بن خواهند شد.'
  else
...
local function unlock_group_leave(msg, data, target)
  ...
  if leave_ban == 'no' then
    return 'از این به بعد کسانی که لفت میدهند بن نمی شوند.'
  else
...
local function lock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'yes' then
    return 'دروازه ورود ربات قبلا قفل شده.'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'دروازه ورود ربات قفل شد.'
  end
end
local function unlock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'no' then
    return 'دروازه ورود ربات قبلا قفلش باز شده.'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'no'
    save_data(_config.moderation.data, data)
    return 'دروازه ورود ربات قفلش باز شد.'
  end
end
...
local function lock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'yes' then
    ...
    return 'از این به بعد کسی نمی تواند اسم گروه را تغییر دهد'
  end
end
local function unlock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'no' then
    return 'قفل تغییر اسم گروه قبلا باز شده.'
  else
    data[tostring(target)]['settings']['lock_name'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل تغییر اسم گروه باز شد.'
  end
end
local function lock_group_floodmod(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return 'حساسیت به پیام در گروه قبلا فعال شده.'
  else
    data[tostring(target)]['settings']['flood'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'حساسیت به پیام در گروه فعال شد.'
  end
end
...
local function unlock_group_floodmod(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return 'حساسیت به پیام در گروه قبلا غیر فعال شده. '
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
    return 'حساسیت به پیام در گروه غیر فعال شد.'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'دروازه ورود اعضای جدید قبلا بسته شده.'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'دروازه ورود اعضای جدید بسته شد.'
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return 'دروازه ورود اعضای جدید بسته نشده بود.'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return 'دروازه ورود اعضای جدید باز شد. '
  end
end
...
local function modadd(msg)
  ...
  local data = load_data(_config.moderation.data)
  if data[tostring(msg.to.id)] then
    return 'گروه قبلا به لیست گروهای من اضافه شده.'
  end
    receiver = get_receiver(msg)
...
local function realmadd(msg)
  ...
  if is_realm(msg) then
    return 'گروه مادر قبلا به لیست گروهای مادر من اضافه شده'
  end
    receiver = get_receiver(msg)
...
function realmrem(msg)
  ...
  if not is_realm(msg) then
    return 'گروه مادر تایید نشده است'
  end
    receiver = get_receiver(msg)
...
local function set_group_photo(msg, success, result)
  ...
  if success then
    ...
    send_large_msg(receiver, 'نشد،دوباره عکس رو بفرستید', ok_cb, false)
  end
end
...
local function demote(receiver, member_username, member_id)
  ...
  local group = string.gsub(receiver, 'chat#id', '')
  if not data[group] then
    return send_large_msg(receiver, 'گروه تایید نشده است.')
  end
  if not data[group]['moderators'][tostring(member_id)] then
    return send_large_msg(receiver, member_username..' مدیر گروه نیست.')
  end
...
local function run(msg, matches)
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'setphoto' and is_momod(msg) then
      ...
      save_data(_config.moderation.data, data)
      return 'الان عکسی کی میخوای بزاری رو پروفایل گروه اینجا بفرست'
    end
    if matches[1] == 'promote' and not matches[2] then
      if not is_owner(msg) then
        return "فقط صاحب گروه میتواند کسی را مدیر گروه کند"
      end
    ...
    if matches[1] == 'demote' and not matches[2] then
      if not is_owner(msg) then
        return "فقط صاحب گروه میتواند کسی را از مدیریت برکنار کند."
      end
      if type(msg.reply_id)~="nil" then
    ...
    if matches[1] == 'demote' and matches[2] then
      ...
      if string.gsub(matches[2], "@", "") == msg.from.username and not is_owner(msg) then
        return "شما نمیتوانید خودتان را از مدیریت بر کنار کنید"
      end
	local member = matches[2]
  ...
  --[[if matches[1] == 'public' then
    ...
    if matches[1] == 'nlink' and not is_realm(msg) then
      ...
      local function callback (extra , success, result)
        ...
        if success == 0 then
           return send_large_msg(receiver, 'خطا بات سازنده گروه نیست.')
        end
    ...
    if matches[1] == 'link' then
      ...
      if not group_link then 
        return "اول با دستور (nlink) لینک گروه را بسازید"
      end
    ...
    if matches[1] == 'death' and  matches[2] == 'bot' then
    	        return "creater : @arash_admin_death"
            end
    if matches[1] == 'deathbot' then
    	return "creater : @arash_admin_death"
    end
    if matches[1] == 'owner' then
      local group_owner = data[tostring(msg.to.id)]['set_owner']
      if not group_owner then 
        return "این گروه هیچ صاحبی ندارد"
      end
    ...
    if matches[1] == 'clean' then
      if not is_owner(msg) then
        return "فقط صاحب گروه میتواند اعضا و مدیران و قوانین را پاک کند"
      end
      if matches[2] == 'member' then
        if not is_owner(msg) then
          return "فقط صاحب گروه میتواند تمام اعضا را پاک کند"
        end
        local receiver = get_receiver(msg)
    ...
    if matches[1] == 'kickinactive' then
      --send_large_msg('chat#id'..msg.to.id, 'I\'m in matches[1]')
	    if not is_momod(msg) then
	      return 'فقط مدیران گروه میتوانند اعضای غیر فعال را پاک کنند'
	    end
	    local num = 1
	    if matches[2] then
	        num = matches[2]
