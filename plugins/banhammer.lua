...
--vardump(extra)
      ...
      local receiver = "chat#id"..chat_id
       if get_cmd == "kick" then
         if member_id == from_id then
             return send_large_msg(receiver, "شما نمی توانید خودتان را اخراج کنید")
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            return send_large_msg(receiver, "شما نمی توانید مدیران گروه صاحب گروه و مدیران بات را اخراج کنید")
         end
         return kick_user(member_id, chat_id)
      elseif get_cmd == 'ban' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
          return send_large_msg(receiver, " شما نمی توانید مدیران گروه صاحب گروه و مدیران بات را بن کنید")
        end
      ...
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از همه گروهام بن شد.')
        return banall_user(member_id, chat_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از همه گروهام از بن در اومد.')
    ...
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(matches[2], '^%d+$') then
        ...
        if not is_admin(msg) and is_momod2(matches[2], msg.to.id) then
          	return "شما نمی توانید مدیران گروه صاحب گروه و مدیران ربات را بن کنید"
        end
...
if matches[1]:lower() == 'kick' then
	...
	if string.match(matches[2], '^%d+$') then
		...
		if not is_admin(msg) and is_momod2(matches[2], msg.to.id) then
			return "شما نمی توانید مدیران گروه صاحب گروه مدیران بات را اخراج کنید"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "شما نمی توانید خودتان را اهراج کنید!"
		end
      		local user_id = matches[2]
      		local chat_id = msg.to.id
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then 
        	...
        	return 'کاربر ['..user_id..' ] از همه گروهام از بن در امد.'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'unbanall',
