...
local function create_realm(msg)
        ...
        if is_sudo(msg) or is_realm(msg) and is_admin(msg) then
                local group_creator = msg.from.print_name
                create_group_chat (group_creator, group_name, ok_cb, false)
                return 'گپ مادر [ '..string.gsub(group_name, '_', ' ')..' ] ساخته شد.'
        end
end
...
local function set_rules(msg, data, target)
    ...
    local data_cat = 'rules'
        ...
        save_data(_config.moderation.data, data)
        return 'قوانین گروه تغییر داده شد به:\n'..rules
end
...
local function admin_promote(msg, admin_id)
        ...
        if data[tostring(admins)][tostring(admin_id)] then
                return admin_name..' قبلا مدیر شده 🎗 .'
        end
...
local function admin_demote(msg, admin_id)
    ...
    local data = load_data(_config.moderation.data)
        ...
        if not data[tostring(admins)][tostring(admin_id)] then
                return admin_id..' مدیر نیست.'
        end
        data[tostring(admins)][tostring(admin_id)] = nil
        save_data(_config.moderation.data, data)
        return admin_id..' از مدیریت برکنار شد.'
end
...
local function realms_list(msg)
    local data = load_data(_config.moderation.data)
        ...
        if not data[tostring(realms)] then
                return 'هیچ گروه مادری وجود ندارد فعلا'
        end
        local message = 'لیست گروهای مادر:\n'
...
local function admin_user_demote(receiver, member_username, member_id)
    local data = load_data(_config.moderation.data)
        ...
        save_data(_config.moderation.data, data)
        return send_large_msg(receiver, 'مدیر '..member_username..' از مدیریت برکنار شد.')
end
...
function run(msg, matches)
    ...
    if matches[1] == 'ساخت گپ مادر' and matches[2] then
        group_name = matches[2]
        group_type = 'realm'
	...
	if matches[2] then if data[tostring(matches[2])] then
              ...
              if matches[1] == 'set' then
                ...
                if matches[1] == 'kill' and matches[2] == 'realm' then
                  ...
                  if is_realm(msg) then
                     ...
                     return 'خطا : گروه مادر '..matches[3]..' پیدا نشد' 
                    end
                 end
		...
		if matches[1] == 'removeadmin' then
			if string.match(matches[2], '^%d+$') then
				local admin_id = matches[2]
				print("کاربر "..admin_id.." از مدیریت برکنار شد")
